...
- **配信形式の補足**: getdesign.md は 2026-04 時点で DESIGN.md 公式仕様（フロントマター付き Markdown）への移行を進めており、配信される 70 サイトのうち 12 サイトは新仕様、58 サイトは依然 legacy prose（`# 見出し` + `## 1. Visual Theme & Atmosphere` 構造）。upstream が将来追加で公式仕様化した場合は `bun run scripts/check-diff-upstream.ts getdesign-md` で SHA 不一致を検知して PR で再追従できる
...
- **Modifications**: 70 サイトのうち **58 サイトを DESIGN.md 公式仕様（frontmatter YAML + Markdown body）に再構成済み**（Issue #31）。upstream の legacy prose から得た design tokens（colors / typography / spacing / radius / components）を frontmatter に構造化、Markdown body は短縮版（Overview / Colors / Typography / Layout / Elevation & Depth / Shapes / Components / Do's and Don'ts / Agent Prompt Guide の 9 セクション）にリライト。English 向けルール（brand font chain は忠実転記、display 系の負 letter-spacing は brand identity として保持、alpha 付き color は不透明 6 桁 hex に近似変換し本文で原値を注記）を適用。`npx @google/design.md@latest lint` で **0 errors** 達成。残り 12 サイト（airbnb / airtable / apple / binance / bmw / bmw-m / bugatti / cal / claude / clay / clickhouse / cohere）は upstream 自身が公式仕様で配信しているものを忠実に vendor、SHA256 一致。`scripts/check-diff-upstream.ts getdesign-md` は変換した 58 件を「changed」、未改変 12 件を「unchanged」として報告（drift 永続化 — VoltAgent が将来公式仕様に追加移行した場合の再追従の選択肢を残すため REPOS には残置）
