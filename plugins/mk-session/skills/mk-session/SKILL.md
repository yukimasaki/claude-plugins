...
| 完了条件 | タブとエージェントが立ち、起動プロンプトを渡せていること（要約が出たかはタブで確かめる） | 親 → 子 → 親 の往復メッセージが通ること |
...
- モード（`MK_SESSION_RESULT` の `mode`）。既定の `standby` なら「起動したセッションは
  Issue を読んで指示待ちで止まる」と書き、**次の指示はユーザーが出すもの**として渡す。
  スクリプトは要約が出たかまでは見ないので、要約が出ているかはタブで確かめる。
  呼び出し元が勝手に作業指示を送らない
...
- **権限確認を飛ばす起動は既定では行わない。** `launchArgs` に書くか `--yolo` を付けたときだけ。
  権限確認ありのまま起動すると、既定の待機モードでは子が `gh issue view` の実行で許可待ちになって
  要約が出ず（スクリプトは要約を見ないので exit 0 で返る）、統括モードでは初期プロンプトの
  `send.sh` で止まって疎通確認がタイムアウトする。自動で最後まで通したいリポジトリでは
