...
3. スクリプトは **ファイル書き換えをしない**。`.design-studio/.upstream-cache/{repo}/`
   が上書きされ、`references/.upstream-state.json` の当該 repo エントリが
   最新にマーキングされる
4. **bootstrap 判定**: 実行前に `references/.upstream-state.json` が空 `{}`
...
- 次のアクション提案:
  ...
  - `references/.upstream-state.json` の diff をレビューして commit 化
    （ユーザーの commit 判断は手動）
...
> Claude: 承知。反映します:
  ...
  .upstream-state.json も更新済み（スクリプトが実行時に更新）。
  変更点をギャラリーで確認するなら `/design-director serve`。
  コミットはそちらで手動にお願い（`git add` + `git commit`）。
```
...
| state.json の zod 検証エラー | state ファイルの破損を検知。ユーザーに `references/.upstream-state.json` の内容確認を依頼、必要なら `{}` にリセット |
...
- Q7-5: `.upstream-state.json` で SHA256 比較
