...
| `/design-director update` | upstream（awesome-claude-design / open-codesign）から更新取り込み | [`references/commands/update.md`](references/commands/update.md) |
...
| `/design-director status` | 現在の状態（dev server / 最終 update / プロジェクト数 / memory.md） | [`references/commands/status.md`](references/commands/status.md) |
...
├── .upstream-cache/              # upstream リポのクローン（.gitignored）
└── .upstream-state.json          # 同期状態（hash 比較用、.gitignored）
```
...
> Claude: sync スクリプト実行 → 差分サマリ提示 → 取り込み承認待ち
```
...
| `/design-director update` が初回実行で全 87 ファイルを「新規」と分類 | `.design-studio/.upstream-state.json` が空 `{}` のため。bootstrap モード（state だけ初期化、実ファイルは触らない）で進める。`update.md` ステップ 2-4 参照 |
