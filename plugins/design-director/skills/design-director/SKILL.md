...
- `design-md/*.md`: ファイル冒頭の **フロントマター `name:` / `description:` フィールド**
  ...
  本リポジトリの 110 サイトはすべて公式仕様で揃っている — japanese-* 25 サイトは
  upstream の kzhrknt（legacy prose 配信）から取り込んだものを、本リポジトリで
  frontmatter 形式に再構成済み（upstream とは SHA drift 状態、Issue #28）。
  upstream（getdesign.md）に新規 prose ファイルが将来取り込まれた場合の
