...
`references/` 配下は open-codesign の prompts / design-skills / builtin-skills、getdesign.md（DESIGN.md 公式仕様）の design-md 70 サイト、rohitg00/awesome-claude-design の prompt-packs / recipes / family 体系の継承、本リポジトリ独自変換の design-md/ 15 サイト（7 単体 + remix 8）と recipes 3 件で構成される vendored 資産（design-md 合計 85 サイト）。
...
- `design-md/*.md`: ファイル冒頭の **フロントマター `name:` / `description:` フィールド**
  ...
  upstream（getdesign.md）の移行中につき、一部サイトはまだ
  **legacy prose 形式**（`# 見出し` + `## 1. Visual Theme & Atmosphere` 構造）で
  配信されている — その場合は `# 見出し` 直後の段落をシグナルとして読む
...
`/design-director [subcommand] [args]` の形式で呼び出す。**コア 4 種 + 補助
4 種 = 計 8 種**。本セクションは概要表のみで、各コマンドの詳細ワークフロー
